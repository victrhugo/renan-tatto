...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // In a real app, you'd verify against an API endpoint that checks process.env.ADMIN_PASSWORD
    // For simplicity in this architecture, we do a basic check (Note: client-side env exposed is not secure, 
    // but we can use a server action to verify. Let's use a simple server action or API route).
    // Wait, to keep it simple and truly secure, we should verify via a Server Action.
    ...
    <div className="min-h-screen bg-black-deep flex items-center justify-center relative overflow-hidden">
      ...
      >
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button
            type="submit"
            className="w-full py-4 bg-gradient-gold text-black-deep uppercase tracking-[0.2em] text-sm font-[family-name:var(--font-oswald)] font-medium hover:shadow-[0_0_20px_rgba(212,175,55,0.3)] transition-all duration-300"
          >
            Entrar
          </button>
        </form>
